...
class TestThreadLocalRegistryManagement:
    ...
    def test_get_current_registry_creates_instance(self) -> None:
        ...
        # Ensure we start with no registry in thread-local using the factory API
        storage = credential_patterns.ThreadLocalRegistryStorageFactory.get_storage()
        storage.clear_registry()
