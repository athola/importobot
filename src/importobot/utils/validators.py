"""Validation utilities and decorators for input validation."""
...
def validate_not_empty(value: Any, param_name: str) -> None:
    ...
    """
    if not value:
        raise exceptions.ValidationError(f"{param_name} cannot be empty")

    if isinstance(value, str) and not value.strip():
        raise exceptions.ValidationError(f"{param_name} cannot be empty or whitespace")
