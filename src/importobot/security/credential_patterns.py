...
class ThreadLocalRegistryStorageFactory:
    """Factory for _ThreadLocalRegistryStorage with lazy initialization.
    ...
    Example:
        ...
        storage = ThreadLocalRegistryStorageFactory.get_storage()

        # For testing: create isolated storage
        test_storage = ThreadLocalRegistryStorageFactory.create_storage()
    """

    _instance: _ThreadLocalRegistryStorage | None = None
    _lock = threading.Lock()

    @classmethod
    def get_storage(cls) -> _ThreadLocalRegistryStorage:
        """Get the default _ThreadLocalRegistryStorage instance.

        Uses lazy initialization with double-checked locking for thread safety.

        Returns:
            The default _ThreadLocalRegistryStorage instance
        """
        if cls._instance is None:
            with cls._lock:
                if cls._instance is None:
                    cls._instance = _ThreadLocalRegistryStorage()
        return cls._instance

    @classmethod
    def create_storage(cls) -> _ThreadLocalRegistryStorage:
        """Create a new isolated _ThreadLocalRegistryStorage instance.

        Useful for testing or isolated execution contexts.

        Returns:
            A new _ThreadLocalRegistryStorage instance
        """
        return _ThreadLocalRegistryStorage()
...
class CredentialRegistryContext:
    ...
    def __init__(self, registry: CredentialPatternRegistry | None = None):
        ...
        self._storage = ThreadLocalRegistryStorageFactory.get_storage()
        self._has_context = self._storage.has_registry()
        self._previous_registry = self._storage.get_registry()

    def __enter__(self) -> CredentialPatternRegistry:
        """Enter context and set registry in thread-local storage."""
        self._storage.set_registry(self.registry)
        return self.registry

    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        """Exit context and restore previous registry."""
        if self._has_context:
            self._storage.set_registry(self._previous_registry)
        else:
            self._storage.clear_registry()
...
def get_current_registry() -> CredentialPatternRegistry:
    ...
    """
    storage = ThreadLocalRegistryStorageFactory.get_storage()
    registry = storage.get_registry()
    if registry is None:
        registry = CredentialPatternRegistry()
        storage.set_registry(registry)
    return registry
